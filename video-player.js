...
// Automatically hide controls when the user is idle
player.on("userinactive", () => {
  player.addClass("vjs-hide-controls");
});

// Show controls when the user becomes active
player.on("useractive", () => {
  player.removeClass("vjs-hide-controls");
});

// Register a new button component
...
// // Register a new button component
// const VolumeButton = videojs.getComponent("Button");

// class CustomVolumeButton extends VolumeButton {
//   constructor(player, options) {
//     super(player, options);
//     this.controlText("Volume"); // Set accessible control text
//   }

//   createEl() {
//     const el = super.createEl("button", {
//       className: "vjs-custom-volume-control vjs-control",
//     });
...
// // Register the custom volume button
...
//   // Remove the default fullscreen control
//   player1.controlBar.fullscreenToggle.hide();

//   // Add the custom fullscreen button
//   player1.controlBar.addChild("CustomFullscreenButton", {});
//   player1.controlBar.addChild("CustomVolumeButton", {}, 1);
// });
const player1 = videojs("my-video", {
  controls: true,
  autoplay: false,
  ...
  controlBar: {
    children: {
      pictureInPictureToggle: false, // Disable the Picture-in-Picture button
    },
  },  
});
...
player1.ready(function () {
  ...
  const volumeControl = player1.controlBar.volumePanel; // Access the volume panel
  if (volumeControl) {
    const volumeButton = volumeControl.el().querySelector(".vjs-mute-control"); // Access the mute/unmute button
    if (volumeButton) {
      // Replace the button's inner HTML with the custom SVG
